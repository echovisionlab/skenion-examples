# Unsupported Pre-Consolidation Compatibility Fixtures

This directory is quarantined unsupported pre-consolidation material for the
older runtime-level graph v0.1 resolution shape. It is not the active product
model for examples, tutorials, help, live help, collaboration, or authoring
fixtures.

Active product fixtures live under `compatibility/v0.1`, `projects/v0.1`, and
`tutorials/v0.1`.
...
Audio clock-domain examples are retained here only as unsupported
pre-consolidation reference material. They are excluded from active Examples CI
and must not be used as positive current-version smoke coverage.
...
from active Examples CI and must not be used as positive current-version smoke
coverage.
