...
PROJECT="compatibility/unsupported/pre-consolidation-v0.1/projects/valid/fullscreen-shader-uniform.project.json"
PATCH="compatibility/unsupported/pre-consolidation-v0.1/patches/valid/set-float-value-to-0-8.patch.json"
...
python3 -c 'import json, sys; r=json.loads(sys.argv[1]); assert r["ok"] is True; assert r["applied"] is True; assert next(n for n in r["snapshot"]["project"]["graph"]["nodes"] if n["id"] == "value_1")["params"]["value"] == 0.8' "${PATCH_RESPONSE}"
